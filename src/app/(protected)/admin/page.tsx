...
import { Trash2, Edit, Plus, Users, Calendar, Mail, MapPin } from 'lucide-react';
      ...
      const updatedUsers = users.map(u =>
        u.id === editingAdmin.id
          ? {
              ...
              name: `${data.lastName || u.lastName}, ${data.firstName || u.firstName} ${data.middleName || u.middleName || ''}`.trim(),
              email: data.email || u.email,
              gender: data.gender || u.gender,
              birthday: data.birthday || u.birthday,
              assignedBarangayId: data.assignedBarangayId || u.assignedBarangayId,
              updatedAt: new Date().toISOString()
            }
          : u
      );

      localStorage.setItem('als_users', JSON.stringify(updatedUsers));
      await loadAdmins();
      setIsEditDialogOpen(false);
      setEditingAdmin(null);
      editForm.reset();
    } catch (error) {
      setSubmitError(error instanceof Error ? error.message : 'Failed to update admin');
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">ADMIN MANAGEMENT</h1>
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto border-4 border-blue-600 dark:border-blue-500 bg-white dark:bg-slate-800">
            <DialogHeader>
              <DialogTitle className="text-blue-800 dark:text-blue-300">Create New Admin Account</DialogTitle>
              <DialogDescription className="text-gray-600 dark:text-gray-300">
                Create a new Master Admin or Regular Admin account for the system
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={createForm.handleSubmit(handleCreateAdmin)} className="space-y-4">
              ...
              <div>
                <Label htmlFor="middleName" className="text-gray-900 dark:text-white">Middle Name</Label>
                <Input
                  id="middleName"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select onValueChange={(value) => createForm.setValue('gender', value)} disabled={isSubmitting}>
                ...
                <div>
                  <Label htmlFor="birthday" className="text-gray-900 dark:text-white">Birthday</Label>
                  <Input
                    id="birthday"
                    type="date"
              ...
              <div>
                ...
                <Select onValueChange={(value) => createForm.setValue('role', value as 'master_admin' | 'admin')} disabled={isSubmitting} defaultValue="admin">
                  <SelectTrigger className="border-2 border-blue-600 dark:border-blue-500 bg-white dark:bg-slate-700 text-gray-900 dark:text-white">
                    <SelectValue placeholder="Select role" />
                  </SelectTrigger>
                  <SelectContent className="bg-white dark:bg-slate-800 border-2 border-blue-600 dark:border-blue-500">
                    <SelectItem value="admin" className="text-gray-900 dark:text-white">Regular Admin</SelectItem>
                    <SelectItem value="master_admin" className="text-gray-900 dark:text-white">Master Admin</SelectItem>
                  </SelectContent>
                </Select>
              ...
              {watchedRole === 'admin' && (
                <div>
                  <Label htmlFor="assignedBarangayId" className="text-gray-900 dark:text-white">Assigned Barangay *</Label>
                <Select onValueChange={(value) => createForm.setValue('assignedBarangayId', value)} disabled={isSubmitting}>
                  <SelectTrigger className="border-2 border-blue-600 dark:border-blue-500 bg-white dark:bg-slate-700 text-gray-900 dark:text-white">
                    <SelectValue placeholder="Select barangay" />
                  </SelectTrigger>
                  <SelectContent className="bg-white dark:bg-slate-800 border-2 border-blue-600 dark:border-blue-500">
                    {barangays.map((barangay) => (
                      <SelectItem key={barangay.id} value={barangay.id} className="text-gray-900 dark:text-white">
                        {barangay.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
        ...
        <CardContent>
          {isLoading ? (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2 text-gray-600 dark:text-gray-300">Loading administrators...</span>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-blue-200 dark:divide-blue-600 border-4 border-blue-600 dark:border-blue-500 rounded-lg">
                ...
                <tbody className="bg-white dark:bg-slate-700 divide-y divide-blue-200 dark:divide-blue-600">
                  {admins.map(admin => (
                    <tr key={admin.id} className="hover:bg-blue-50 dark:hover:bg-slate-600">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge
                          variant={admin.role === 'master_admin' ? 'default' : 'secondary'}
                          className={`${
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto border-4 border-blue-600 dark:border-blue-500 bg-white dark:bg-slate-800">
          <DialogHeader>
            <DialogTitle className="text-blue-800 dark:text-blue-300">Edit Admin Account</DialogTitle>
            <DialogDescription className="text-gray-600 dark:text-gray-300">
          ...
          <form onSubmit={editForm.handleSubmit(handleEditAdmin)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="editFirstName" className="text-gray-900 dark:text-white">First Name *</Label>
                <Input
                  id="editFirstName"
              ...
              <div>
                <Label htmlFor="editLastName" className="text-gray-900 dark:text-white">Last Name *</Label>
                <Input
                  id="editLastName"
            ...
            <div>
              <Label htmlFor="editMiddleName" className="text-gray-900 dark:text-white">Middle Name</Label>
              <Input
                id="editMiddleName"
            ...
            <div>
              <Label htmlFor="editEmail" className="text-gray-900 dark:text-white">Email *</Label>
              <Input
                id="editEmail"
                type="email"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="editGender" className="text-gray-900 dark:text-white">Gender</Label>
                <Select
              ...
              <div>
                <Label htmlFor="editBirthday" className="text-gray-900 dark:text-white">Birthday</Label>
                <Input
                  id="editBirthday"
                  type="date"
            ...
            {editingAdmin?.role === 'admin' && (
              <div>
                <Label htmlFor="editAssignedBarangayId" className="text-gray-900 dark:text-white">Assigned Barangay *</Label>
                <Select
                  onValueChange={(value) => editForm.setValue('assignedBarangayId', value)}
                  disabled={isSubmitting}
